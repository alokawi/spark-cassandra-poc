# cassandra-poc

# Example(s)
Spark File Loader Example - https://github.com/alokawi/cassandra-poc/blob/master/src/main/java/alokawi/poc/spark/fileloader/SparkFileLoaderUtils.java

# References
http://spark.apache.org/docs/latest/sql-programming-guide.html#datasets-and-dataframes
http://spark.apache.org/docs/2.0.0/sql-programming-guide.html

# Output
https://docs.google.com/spreadsheets/d/1i6D7oPXR_xKO0mNwCC9uG_Pn5HD69AZkiBOJm38Y6Sc/edit#gid=1673083041

![Alt text](Video_Drop_Off_Ouput.png?raw=true "Video_Drop_Off_Ouput.png")

# Author
https://www.linkedin.com/in/alokawi

http://blog.alokawinash.in/
